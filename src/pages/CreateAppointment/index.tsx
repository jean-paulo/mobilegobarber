...
const CreateAppointment: React.FC = () => {
  ...
  const handleToggleDatePicker = useCallback(() => {
    /* Altera pro valor inverso do showDatePicker */
  ...
  const handleCreateAppointment = useCallback(async () => {
    try {
      ...
      /* Para garantir que os segundos estarão zerados */
      date.setMinutes(0);
      ...
      Alert.alert('Erro ao criar agendamento', 'Ocorreu um erro ao tentar cadastrar o agendamento, tente novamente.');
    }
  }, [navigate, selectedDate, selectedHour, selectedProvider]);

  const morningAvailability = useMemo(() => availability
    .filter(({ hour }) => hour < 12).map(({ hour, available }) => ({
      hour,
      available,
